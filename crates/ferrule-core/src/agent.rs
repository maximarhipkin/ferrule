...
use crate::guard::{guarded_call, unless_halted, Guard, Guarded, GuardedCall};
    ...
    async fn run_inner(
        ...
        for iteration in 0..self.config.max_iterations {
            ...
            for (i, call) in msg.tool_calls.iter().enumerate() {
                ...
                let (content, ok) = match guarded_call(guard.as_ref(), seen, run).await {
                    Guarded::Ran(Ok(out)) => (out.content, true),
                    Guarded::Ran(Err(e)) => (format!("error: {e}"), false),
                    Guarded::Refused(why) => (format!("refused by ferrule: {why}"), false),
                    Guarded::Halted(why) => {
                        self.emit(
                            &tx,
