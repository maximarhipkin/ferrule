...
fn state_dir(ctx: &ToolContext) -> PathBuf {
    ctx.workspace.join(".agentrust")
}

/// `write_todos` — the agent maintains its own task list in
/// `.agentrust/TODO.md`. Forces explicit planning, and the file doubles as a
    ...
    fn definition(&self) -> ToolDefinition {
        ToolDefinition {
            ...
            description: "Write the current task list as markdown (overwrites). \
                          ...
                          complete. Persisted to .agentrust/TODO.md in the workspace."
                .into(),
            parameters: json!({
                "type": "object",
                "properties": {
...
/// `log_diary` — append-only agent diary at `.agentrust/diary.md`. The point
    ...
    fn definition(&self) -> ToolDefinition {
        ToolDefinition {
            ...
            description: "Append a timestamped progress note to the agent diary \
                          (.agentrust/diary.md). Log decisions, blockers, and \
...
mod tests {
    ...
    async fn todos_overwrite_and_count() {
        ...
        let text = std::fs::read_to_string(dir.path().join(".agentrust/TODO.md")).unwrap();
    ...
    async fn diary_appends() {
        ...
        let text = std::fs::read_to_string(dir.path().join(".agentrust/diary.md")).unwrap();
